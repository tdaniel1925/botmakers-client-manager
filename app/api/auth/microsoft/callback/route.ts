 ...
 * Handles OAuth redirect from Microsoft after user grants permission
 */

import { NextRequest, NextResponse } from 'next/server';
import { auth } from '@clerk/nextjs/server';
import { exchangeMicrosoftCode } from '@/lib/email-providers/microsoft-oauth';
...
export async function GET(request: NextRequest) {
  try {
    ...
    if (error) {
      ...
      return NextResponse.redirect(
        new URL(`/platform/emails?error=oauth_failed&provider=microsoft`, request.url)
      );
    }
    ...
    return NextResponse.redirect(
      new URL('/platform/emails?success=microsoft_connected', request.url)
    );
  } catch (error) {
